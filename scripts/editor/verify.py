...
with tempfile.TemporaryDirectory(prefix="verify-", dir="/workspace/.state/editor") as tmp:
    ...
    ipynb_state = root / "ipynb.json"
    nvim, first_start = launch(notebook)
    try:
        sys.path.insert(0, os.environ["XDG_DATA_HOME"] + "/nvim/lazy/ipynb.nvim/rplugin/python3")
        from ipynb_runtime.outputbuffer import OutputBuffer, render_control_chars
        from ipynb_runtime.outputchunks import Output, OutputStatus, TextLnOutputChunk, TextOutputChunk

        assert render_control_chars("abc\bX") == "abX"
        assert render_control_chars("first\n\rsecond") == "first\nsecond"
        output = Output(None)
        ...
        wait(nvim, lambda: nvim.eval("exists(':IpynbInit')") == 2, "ipynb commands")
        ...
        assert nvim.eval("g:ipynb_virt_text_max_lines") == 8
        assert nvim.eval("g:ipynb_output_win_max_height") == 12

        def executions_done(after=0):
            nvim.eval("IpynbTick()")
            kernels = nvim.eval("IpynbRunningKernels()")
            if not kernels:
                return False
            nvim.command(f"silent IpynbSave {nvim.call('fnameescape', str(ipynb_state))} {kernels[0]}")
            outputs = json.loads(ipynb_state.read_text())["cells"]
        ...
        state = json.loads(ipynb_state.read_text()) if ipynb_state.exists() else {"cells": []}
        print("ipynb state:", [(cell["execution_count"], cell["status"], cell["success"])
    ...
    try:
        ...
        first = next(i for i, line in enumerate(nvim.current.buffer[:]) if line.startswith("```python"))
        nvim.current.window.cursor = (first + 2, 0)
        nvim.exec_lua("vim.fn.maparg(vim.g.maplocalleader .. 'o', 'n', false, true).callback()")

        def output_windows():
            return [window for window in nvim.windows
                    if window.valid and nvim.buffers[window.buffer.number].options["filetype"] == "ipynb_output"]

        wait(nvim, lambda: bool(output_windows()), "open saved output")
        output_window = nvim.current.window
        assert nvim.current.buffer.options["filetype"] == "ipynb_output"
        assert nvim.api.win_get_config(output_window.handle)["relative"] == "win"
        line_count = len(nvim.current.buffer)
        self_scroll = output_window.cursor[0]
        assert line_count > nvim.api.win_get_height(output_window.handle)
        nvim.command("normal! G")
        assert output_window.cursor[0] > self_scroll
        nvim.input("\\O")
        wait(nvim, lambda: not output_windows(), "close saved output")
        nvim.command("write")
        assert "BOOK_EDITED_OK" in notebook.read_text()
    ...
    try:
        ...
        assert not nvim.exec_lua("return vim.fn.maparg(vim.g.maplocalleader .. 'o', 'n', false, true).buffer == 1")
        assert not nvim.exec_lua("return vim.fn.maparg(vim.g.maplocalleader .. 'O', 'n', false, true).buffer == 1")
    ...
    try:
        ...
        def new_output():
            ...
            if not kernels:
                return False
            nvim.command(f"silent IpynbSave {nvim.call('fnameescape', str(state))} {kernels[0]}")
